...
const ProfessionalSummaryForm = ({data, onChange, setResumeData}) => {
    ...
    const generateSummary = async ()=>{
        try {
            ...
            const response = await api.post('/api/v1/ai/enhance-pro-sum', {userContent: prompt},{headers:{Authorization: `Bearer ${token}`}})
            // console.log(response.data?.message)
            setResumeData(prev => ({...prev, professional_summary: response.data?.message}))
  ...
  return (
    <div className=' space-x-4'>
        ...
        <div className=' mt-6'>
            ...
            />
            <p className=' text-xs text-gray-500 max-w-4/5 max-auto text-center'>Tip: Keep it concise and highlight your most relevant skills and achievements.</p>
        </div>
