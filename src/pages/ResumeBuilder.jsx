...
const ResumeBuilder = () => {
  ...
  const loadExistingResume = async () => {
   try {
    const {data} = await api.get('/api/v1/resumes/get/'+ resumeId,{headers:{Authorization: `Bearer ${token}`}})
    console.log(data)
    if(data.resume){
  ...
  const changeResumeVisibility = async () => {
    try {
      ...
      const {data} = await api.put('/api/v1/resumes/update', formData, {headers:{Authorization: `Bearer ${token}`}})
  ...
  const saveResume = async ()=>{
    try {
      ...
      // console to check image send to server
      // console.log(resumeData.personal_info.image.name)
      const {data} = await api.put('/api/v1/resumes/update', formData, {headers:{Authorization: `Bearer ${token}`}})
      setResumeData(data.resume)
      toast.success(data.message)
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8">
        <div className="grid lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 max-lg:mt-6">
            <div className=" relative w-full">{/* --buttons--- */}
